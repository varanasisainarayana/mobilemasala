...
const Editposts = () => {
  ...
  const filterCategories = (language) => {
    ...
    const filtered = categories.filter((category) => {
      ...
      return category.language == language && category.categoryType === 1; //
    });
    ...
    if (item && item.categoryId) {
      ...
      if (selectedCategory1) {
        ...
        setFormData((prevData) => ({
          ...prevData,
          categoryName: selectedCategory1.categoryName,
          language: selectedCategory1.language, // Set the language based on the selected category
        }));
        console.log(selectedCategory1, "Hemasundhar345677888");
      }
    }
  }, [item, categories]);

  console.log(filteredCategories);
  console.log(item, "Hemasundhar");
  ...
  const config = {
    ...
    // events: {
    //   onFileUpload: async (file, editor, callback) => {
    //     try {
    ...
    //         callback(fileUrl); // Pass the uploaded file URL to the editor
    //       } else {
  ...
  const handleSubmit = (e) => {
    ...
    // setFormData({
    //   ...formData,
    //   scheduleDate: formData.schedule === "Yes" ? new Date() : null,
    // });
    setLoading(true);
    ...
    fetch(`${BASE_URL}/mm/articles`, {
      ...
      .then((data) => {
        ...
        if (data.status === 200) {
          ...
          setAlert({
            show: true,
            message: "Error editing posts. Please try again.",
            type: "danger",
          });
        }
      })
                  ...
                  {filteredCategories &&
                    filteredCategories.map((category) => (
                      <option
                        ...
                        selected={category.categoryId === formData.categoryId}>
